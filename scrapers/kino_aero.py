...
--------------------------------------------------------
kinoaero.cz/program is server-rendered (no JavaScript needed) and, helpfully,
embeds a schema.org "Event" block as JSON-LD next to every single screening.
That block is our primary source, because it gives us the things that are
painful to scrape from visible text:

    - the exact start time as a timezone-aware ISO timestamp, which means we
      never have to interpret the Czech day headers "Dnes" / "Zítra"
    - the film's language, director, runtime and poster
    - a stable per-screening URL we can use as the "Vstupenky" link
...
today will look like it had one screening all day. The cron job in Milestone 5
should therefore run early in the morning, before the first matinee, or each run
will quietly lose that day's earlier shows.

One nice detail worth knowing: Aero only tags the *exception*. Dubbed screenings
carry a "Dabing" chip; subtitled ones carry nothing at all. There is no "Titulky"
tag anywhere on the page. That matches the app's rule of showing a version chip
only when it deviates from the norm, so an empty `language_version` should be
read as "presented the usual way", not as "we failed to find it".
"""
...
def scrape(html: str | None = None) -> dict:
    ...
    for day_block in soup.find_all("div", class_="program"):
        ...
        for row in day_block.find_all("div", class_="program__info-row"):
            screening = _parse_row(row, fallback_date=day_date)
            if screening:
...
def _parse_row(row, fallback_date: str | None) -> Screening | None:
    ...
    raw_tags = [
        tag.get_text() for tag in row.find_all("span", class_="program__tag")
    ]
    ...
    return Screening(
        ...
        booking_url=_extract_booking_url(row, data),
...
def _extract_booking_url(row, data: dict) -> str:
    """
    The link the app's "Vstupenky" button should open.

    Aero's ticket button is a POST form to the Colosseum ticketing system, which
    we can't turn into a plain link. But the JSON-LD offer carries a normal URL
    that opens this exact screening on kinoaero.cz — good enough to hand the user
    off to the right place, which is all the app promises to do.
    """
    ...
    if projection_id:
        return f"https://kinoaero.cz/?projection={projection_id}"
    return PROGRAM_URL


def _extract_source_id(row) -> str:
    """Aero's own id for this screening, carried on several elements as data-projection."""
